...
describe("getStageBoostBudget", () => {
  it("returns the full ladder for tier 1", () => {
    expect(getStageBoostBudget(1, "round_of_32")).toBe(5);
    expect(getStageBoostBudget(1, "round_of_16")).toBe(3);
    expect(getStageBoostBudget(1, "quarter_final")).toBe(2);
    expect(getStageBoostBudget(1, "semi_final")).toBe(1);
  });

  it("returns the cumulative ladder for lower tiers", () => {
    expect(getStageBoostBudget(2, "quarter_final")).toBe(2);
    expect(getStageBoostBudget(3, "round_of_16")).toBe(3);
    expect(getStageBoostBudget(4, "round_of_32")).toBe(5);
    expect(getStageBoostBudget(4, "round_of_16")).toBe(2);
  });
...
describe("getBoostBudgetMatrix", () => {
  it("matches the spec matrix", () => {
    ...
    expect(matrix[3]).toMatchObject({
      round_of_32: 5,
      round_of_16: 3,
      quarter_final: 1,
      semi_final: 0,
    });
    expect(matrix[4]).toMatchObject({
      round_of_32: 5,
      round_of_16: 2,
      quarter_final: 1,
      semi_final: 0,
